...
public class Robot extends IterativeRobot {
	...
	public void teleopPeriodic() {
		...
		boolean redon = techstick.getRawButton(8);
		boolean greenon = techstick.getRawButton(10);
		boolean blueon = techstick.getRawButton(12);
		boolean redoff = techstick.getRawButton(7);
		boolean greenoff = techstick.getRawButton(9);
		boolean blueoff = techstick.getRawButton(11);
		

		

		boolean autoLockout = driverstick.getTrigger();
		crazyMode = techstick.getRawButton(1);
		...
		if ((level1) && !levelQue) {// if button 3 on either joystick is pressed and has not been held down
		...
		if ((level2) && !levelQue) {// same idea, but for level 2
			levelQue = true;
			levelReq = false;
			downRequest = false;
			level2Req = true;
		...
		if ((down) && !levelQue) {// for lowering elevator
			levelQue = true;
			levelReq = false;
			level2Req = false;
			downRequest = true;
		...
		if (redon) {
			Wire.write(1, 0);// turns leds on
		} if(redoff) {
			Wire.write(1, 1);// turns leds off
		}

		if (greenon) {
			Wire.write(1, 2);// turns leds on
		} if(greenoff) {
			Wire.write(1, 3);// turns leds off
		}

		if (blueon) {
			Wire.write(1, 4);// turns leds on
		} if(blueoff) {
			Wire.write(1, 5);// turns leds off
		}
		...
		/****************************************************************************************************/
		if (driverstick.getRawButton(7) || techstick.getRawButton(5)) {// if button 7 is pressed, lower the piston to
																		// change the shooting angle
			p_Deploy.set(true);
			p_unDeploy.set(false);
			shootswitch = false;
		}
		if (driverstick.getRawButton(8) || techstick.getRawButton(6)) {// if button 8 is pressed, raise the pistons
			p_Deploy.set(false);
			p_unDeploy.set(true);
			shootswitch = true;
		}
		if (ballIn && !autoLockout && m_encoder.getPosition() <= 510 && !lifttopMax) {// if manual override is not activated and the bot has a ball
		...
		if (!ballIn && !autoLockout && m_encoder.getPosition() <=510 && !lifttopMax) {// same as the whole big chunk above but with slightly different
													// values
